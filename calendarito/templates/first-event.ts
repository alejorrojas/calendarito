export interface FirstEventTemplateData {
  userEmail: string;
  eventSummary: string;
  eventDate: string;
  calendarId: string;
...
export function firstEventTemplate(data: FirstEventTemplateData) {
  const { userEmail, eventSummary, eventDate, eventCount } = data;

  return {
    subject: "🗓️ First event created on Calendarito!",
    html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>First Event Created</title>
</head>
          ...
          <tr>
            <td style="background:#ffffff;border-radius:20px;padding:40px 36px;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
              ...
              <table cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                <tr>
                  <td style="background:#0A0A0A;border-radius:999px;padding:6px 16px;">
                    <span style="font-size:13px;font-weight:700;color:#E8E815;letter-spacing:-0.01em;">First event</span>
                  </td>
                </tr>
              </table>
              ...
              <h1 style="margin:0 0 8px;font-family:'Arial Black',Arial,sans-serif;font-size:28px;font-weight:900;letter-spacing:-0.04em;color:#0A0A0A;line-height:1.1;">
                A user just created their first event!
              </h1>
              <p style="margin:0 0 28px;font-size:16px;color:#555555;line-height:1.6;">
                Calendarito is working its magic. ${eventCount} event${eventCount > 1 ? "s" : ""} added to Google Calendar.
              </p>

              <!-- User info box -->
              <table width="100%" cellpadding="0" cellspacing="0"
                style="background:#F2F2F2;border-radius:12px;padding:20px 24px;margin-bottom:16px;">
                <tr>
                  <td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0"
                style="background:#E8E815;border-radius:12px;padding:16px 24px;margin-bottom:28px;">
                <tr>
                  <td>
                    <p style="margin:0;font-size:14px;font-weight:700;color:#0A0A0A;">
                      🎯 They dropped something in and it worked. That's the whole product.
                    </p>
                  </td>
                </tr>
              </table>
